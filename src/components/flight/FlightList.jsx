...
import { Edit, Trash2, Plane, MapPin, Briefcase } from "lucide-react";
...
export default function FlightList({ flights, onEdit, onDelete }) {
  ...
  return (
    <div className="space-y-4">
      {flights.map((flight) => (
        <Card key={flight.id} className="bg-white shadow-lg border-0 hover:shadow-xl transition-shadow duration-300">
                ...
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="font-semibold text-slate-900">
                      {flight.is_simulator ? "Simulator" : flight.aircraft_type === "Custom" ? flight.custom_aircraft_type : flight.aircraft_type}
                    </span>
                    {!flight.is_simulator && (
                      <>
