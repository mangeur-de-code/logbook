...
const AIRCRAFT_TYPES = ["UH-60", "HH-60", "UH-72", "CH-47", "C-12", "AH-64", "AH-64E", "Custom"];

export default function DataManagement({ flights, onDeletionComplete }) {
  const [selectedAircraft, setSelectedAircraft] = useState("");
  const [isProcessing, setIsProcessing] = useState(false);
  const [progress, setProgress] = useState(null); // { current: 0, total: 0, message: "" }
  ...
  const uniqueAircraftTypes = useMemo(() => {
    if (!flights) return [];
    const types = flights.map(f => f.aircraft_type === "Custom" ? f.custom_aircraft_type : f.aircraft_type);
  ...
  const uniqueSimulatorTypes = useMemo(() => {
    ...
    const types = simulatorFlights.map(f => f.aircraft_type === "Custom" ? f.custom_aircraft_type : f.aircraft_type);
    return [...new Set(types)].filter(Boolean).sort();
  }, [flights]);
  ...
  const matchingFlights = useMemo(() => {
    ...
    return flights.filter(flight => {
      const fieldValue = searchField === "aircraft_type" 
        ? (flight.aircraft_type === "Custom" ? flight.custom_aircraft_type : flight.aircraft_type)
        : flight[searchField];
  ...
  const runRateLimitedOperation = async (items, operation, type) => {
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        while (retryCount < maxRetries && !success) {
            try {
                ...
                if (error.response?.status === 429 || error.message?.includes("Rate limit")) {
                    ...
                    const waitTime = Math.min(exponentialDelay + jitter, 15000); // Cap at 15 seconds
        ...
        // Base delay between successful operations (increased)
        if (success) {
    ...
    // Show final message for 3 seconds
    setTimeout(() => {
  ...
  const handleDelete = async () => {
    ...
    const flightsToDelete = flights.filter(f => (f.aircraft_type === "Custom" ? f.custom_aircraft_type : f.aircraft_type) === selectedAircraft);
  ...
  const handleBatchUpdate = async () => {
    ...
    if (replaceField === "aircraft_type" && newAircraftType === "Custom" && !customAircraftType.trim()) {
    ...
    const updateOperation = async (flight) => {
        ...
        if (convertToSimulator) {
          ...
          if (newAircraftType === "Custom") {
            updates.aircraft_type = "Custom";
  ...
  const flightsToDeleteCount = selectedAircraft 
    ? flights.filter(f => (f.aircraft_type === "Custom" ? f.custom_aircraft_type : f.aircraft_type) === selectedAircraft).length
    : 0;
              ...
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  ...
                  {convertToSimulator ? " These flights will be converted to simulator sessions." : 
                   replaceField === "aircraft_type" ?
                     ` The aircraft type will be changed to "${newAircraftType === "Custom" ? customAircraftType : newAircraftType}".` :
