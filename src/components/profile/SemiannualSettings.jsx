...
import { Calendar, Settings, Plus, Trash2, Save, Computer, Edit, RotateCw, GripVertical } from "lucide-react";
...
export default function SemiannualSettings({ user, onUpdate, saving, onDirtyChange }) {
  ...
  const handleSaveCustomField = () => {
    if (!fieldFormData.name.trim() || fieldFormData.required_hours <= 0) {
        // Basic validation
        return;
    }
    ...
    if (editingField) {
      ...
      setSettings(prev => ({
        ...prev,
        custom_fields: prev.custom_fields.map(f => f.id === editingField.id ? fieldFormData : f)
      }));
    } else {
      // Add new field
      const newField = { ...fieldFormData, id: generateId() };
      setSettings(prev => ({
        ...prev,
                  ...
                  {(provided) => (
                    ...
                    >
                      {settings.custom_fields.map((field, index) => {
                        const customFieldAircraftType = field.is_simulator ? settings.simulator_aircraft_type : settings.aircraft_type;
                        const seatOptsForField = (customFieldAircraftType === 'AH-64D' || customFieldAircraftType === 'AH-64E') ? SEAT_OPTIONS.apache : SEAT_OPTIONS.default;
                        const seatLabel = seatOptsForField.find(opt => opt.value === field.seat_position)?.label || field.seat_position;
                                ...
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    ...
                                    <span className="text-slate-600 ml-2">
                                      ...
                                      {field.seat_position ? `, Seat: ${seatLabel}` : ''}
                                      {field.is_simulator ? ', Simulator' : ', Aircraft'})
                                    </span>
                                  </div>
                                </div>
