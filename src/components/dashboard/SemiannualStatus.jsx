...
const getCurrentPeriod = (settings) => {
  ...
  if (settings.period_one_start && settings.period_one_end) {
    const p1_start = new Date(settings.period_one_start);
    const p1_end = new Date(settings.period_one_end);
  ...
  if (settings.period_two_start && settings.period_two_end) {
    const p2_start = new Date(settings.period_two_start);
    const p2_end = new Date(settings.period_two_end);
      ...
      settings.custom_fields?.forEach(field => {
        ...
        targetFlights.forEach(flight => {
          flight.hour_breakdown?.forEach(breakdown => {
            const modeMatch = !field.mode || breakdown.mode === field.mode;
